...
struct FontFamilySpecificationCoreText {
  ...
  func fontRanges(fontDescription: FontDescription) -> FontRanges {
    ...
    let font = SVGUIView.preparePlatformFont(
      originalFont: unrealizedFont,
    ...
    let platformData = FontPlatformData(
      font: font, size: size, orientation: fontDescription.orientation, widthVariant: fontDescription.widthVariant,
  ...
  static func computeNecessarySynthesis(font: CTFont, fontDescription: FontDescription, isPlatformFont: Bool = false) -> SynthesisPair {
    ...
    let needsSyntheticBold =
      fontDescription.hasAutoFontSynthesisWeight
      && desiredTraits.contains(.traitBold)
      && !actualTraits.contains(.traitBold)
    let needsSyntheticOblique =
