...
class FontCache {
  ...
  func createFontPlatformData(fontDescription: FontDescription, familyName: String, fontCreationContext: FontCreationContext) -> FontPlatformData? {
    ...
    guard
      let font = SVGUIView.fontWithFamily(
        fontDatabase: fontDatabase, familyName: familyName,
    ...
    let (syntheticBold, syntheticOblique) = FontFamilySpecificationCoreText.computeNecessarySynthesis(
      font: font,
      fontDescription: fontDescription
    ).boldObliquePair
    let platformData = FontPlatformData(
      font: font, size: size,
  ...
  func cachedFontPlatformData(
    fontDescription: FontDescription, familyName: String,
    fontCreationContext: FontCreationContext, checkingAlternateName: Bool = false
  ) -> FontPlatformData? {
    let key = FontPlatformDataCacheKey(
      descriptionKey: .init(description: fontDescription),
    ...
    guard let value = fontDataCaches.platformData(for: key) else {
      ...
      if value == nil, !checkingAlternateName {
        if let alternateName = Self.alternateFamilyName(familyName: familyName) {
          if let alternateData = cachedFontPlatformData(
            fontDescription: fontDescription, familyName: alternateName,
  ...
  func similarFont(description: FontDescription, familyName: String) -> Font? {
    ...
    if SVGUIView.equalLettersIgnoringASCIICase(string: familyName, literal: "monaco") || SVGUIView.equalLettersIgnoringASCIICase(string: familyName, literal: "menlo") {
  ...
  func fontForFamily(
    fontDescription: FontDescription, familyName: String,
    fontCreationContext: FontCreationContext = .init(), checkingAlternateName: Bool = false
  ) -> Font? {
    guard
      let platformData = cachedFontPlatformData(
        fontDescription: fontDescription, familyName: familyName,
  ...
  func lastResortFallbackFont(fontDescription: FontDescription) -> Font {
    ...
    let platformData = FontPlatformData(
      font: font, size: fontDescription.computedSize,
      orientation: fontDescription.orientation, widthVariant: fontDescription.widthVariant,
      syntheticBold: syntheticBold, syntheticOblique: syntheticOblique)
    return fontForPlatformData(platformData: platformData)
  }
