...
struct SVGFeGaussianBlurElement: SVGElement, SVGFilterApplier {
  ...
  func applyAccelerated(srcBuffer: inout vImage_Buffer, destBuffer: inout vImage_Buffer, kernelSize: UInt32) {
    let tmpBufferSize = vImageBoxConvolve_ARGB8888(
      &srcBuffer,
      &destBuffer,
      nil,
      0,
  ...
  func applyUnaccelerated(srcBuffer: inout vImage_Buffer, destBuffer: inout vImage_Buffer, kernelSizeX: UInt32, kernelSizeY: UInt32, context _: SVGContext) {
    ...
    while i < 3 {
      if kernelSizeX > 0 {
        ...
        boxBlur(
          srcBuffer: &srcBuffer, destBuffer: &destBuffer, dx: Int(kernelSizeX),
      ...
      if kernelSizeY > 0 {
        ...
        boxBlur(
          srcBuffer: &srcBuffer, destBuffer: &destBuffer, dx: Int(kernelSizeY),
  ...
  private func boxBlur(
    srcBuffer: inout vImage_Buffer, destBuffer: inout vImage_Buffer, dx: Int, dxLeft: Int, dxRight: Int,
