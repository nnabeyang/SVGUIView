...
struct SVGFeBlendElement: SVGElement, SVGFilterApplier {
    ...
    func apply(srcImage: CGImage, inImage: CGImage, clipRect: inout CGRect,
               filter: SVGFilterElement, frame: CGRect, effectRect: CGRect, opacity: CGFloat, cgContext: CGContext, context: SVGContext, results: [String: CGImage], isFirst _: Bool) -> CGImage?
    {
        guard var format = vImage_CGImageFormat(bitsPerComponent: srcImage.bitsPerComponent,
            ...
            let image1 = inputImage(input: input, format: &format,
                                    results: results, srcBuffer: &srcBuffer, inputBuffer: &inputBuffer),
            let image2 = inputImage(input: input2, format: &format,
                                    results: results, srcBuffer: &srcBuffer, inputBuffer: &inputBuffer) else { return nil }
    ...
    private func inputImage(input: SVGFilterInput?, format: inout vImage_CGImageFormat, results: [String: CGImage],
                            srcBuffer: inout vImage_Buffer, inputBuffer: inout vImage_Buffer) -> CGImage?
    {
        ...
        return vImageCreateCGImageFromBuffer(&destBuffer,
                                             &format,
                                             { _, _ in },
                                             nil,
