...
struct CSSParser {
  ...
  mutating func parseSelector() -> Result<CSSSelector, CSSParseError> {
    ...
    switch token {
    case .delimiter(let ch):
      switch ch {
      case UInt8(ascii: "."):
        guard case .ident(let name) = tokenizer.next() else { return .failure(.invalid) }
  ...
  mutating func parseDeclarations<T>(tokenizer: inout CSSTokenizer<T>) -> [CSSValueType: CSSDeclaration] {
    ...
    while true {
      ...
      case .ident:
        ...
        if case .success(let declaration) = result, declarations[declaration.type] == nil {
  ...
  mutating func parseDeclaration<T>(tokenizer: inout CSSTokenizer<T>) -> Result<CSSDeclaration, CSSParseError> {
    ...
    case .fill:
      ...
      case .function(let name):
        ...
        switch fname {
        case .rgb:
          guard case .number(let r) = tokenizer.next() else { return .failure(.invalid) }
          guard case .comma = tokenizer.next() else { return .failure(.invalid) }
          guard case .number(let g) = tokenizer.next() else { return .failure(.invalid) }
          guard case .comma = tokenizer.next() else { return .failure(.invalid) }
          guard case .number(let b) = tokenizer.next() else { return .failure(.invalid) }
        ...
        case .rgba:
          ...
          guard case .number(let b) = tokenizer.next() else { return .failure(.invalid) }
          guard case .comma = tokenizer.next() else { return .failure(.invalid) }
          guard case .number(let a) = tokenizer.next() else { return .failure(.invalid) }
          guard case .rightParenthesis = tokenizer.next() else { return .failure(.invalid) }
    ...
    case .height, .width, .x, .y:
      guard case .dimension(let value, let unit) = tokenizer.next() else { return .failure(.invalid) }
    ...
    case .clipPath:
      guard case .url(let url) = tokenizer.next(), url.hasPrefix("#") else { return .failure(.invalid) }
  ...
  mutating func parseTransform<T>(tokenizer: inout CSSTokenizer<T>) -> Result<[any TransformOperator], CSSParseError> {
    ...
    while true {
      ...
      guard case .function(let fname) = tokenizer.next() else { return .failure(.invalid) }
      guard let type = TransformType(rawValue: fname) else { return .failure(.invalid) }
      switch type {
      case .scale:
        guard case .number(let x) = tokenizer.next() else { return .failure(.invalid) }
      ...
      case .rotate:
        guard case .number(let angle) = tokenizer.next() else { return .failure(.invalid) }
        if case .comma = tokenizer.peek() { _ = tokenizer.next() }
        switch tokenizer.next() {
        case .rightParenthesis:
        ...
        case .number(let x):
          if case .comma = tokenizer.peek() { _ = tokenizer.next() }
          guard case .number(let y) = tokenizer.next() else { return .failure(.invalid) }
          guard case .rightParenthesis = tokenizer.next() else { return .failure(.invalid) }
      ...
      case .matrix:
        ...
        guard case .number(let b) = tokenizer.next() else { return .failure(.invalid) }
        if case .comma = tokenizer.peek() { _ = tokenizer.next() }
        guard case .number(let c) = tokenizer.next() else { return .failure(.invalid) }
        if case .comma = tokenizer.peek() { _ = tokenizer.next() }
        guard case .number(let d) = tokenizer.next() else { return .failure(.invalid) }
        if case .comma = tokenizer.peek() { _ = tokenizer.next() }
        guard case .number(let tx) = tokenizer.next() else { return .failure(.invalid) }
        if case .comma = tokenizer.peek() { _ = tokenizer.next() }
        guard case .number(let ty) = tokenizer.next() else { return .failure(.invalid) }
        guard case .rightParenthesis = tokenizer.next() else { return .failure(.invalid) }
